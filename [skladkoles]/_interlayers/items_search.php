...
if( !empty($supplied_data) ) {
    $items_search = $supplied_data;
}

$xml_items_search = $_BOOT->involve_object("XML_Handler")->get_xml(
    basename(__FILE__, '.php'),
    $_AREA->{$C_E::_LANGUAGE}
);

if( empty($items_search) ): ?>
    <p class="empty"><?=$xml_items_search->no_data?></p>
<?php else:
    $data_landmark = json_encode(
        array(
            ...
            'AR_location' => "items_search",
            'AR_method'   => "get_items_search"
        ),
        JSON_UNESCAPED_SLASHES
    );
    ?>
    <div class="lift-data" data-landmark='<?=$data_landmark?>'>
        <?php foreach($items_search as $value): ?>
            <div class="item-container lift">
                <div class="item">
                    ...
                    <?php elseif( $value['type'] == 'exclusive_rims' || $value['type'] == 'exclusive_tyres' ): ?>
                        <div class="ribbon">
                ...
                <a href="<?=$this->get_current_link("item_details/{$value['type']}/{$value['id']}")?>"><i class="fa fa-list-ul"></i><?=$xml_items_search->button_detailed?></a>
            </div>
        <?php endforeach; ?>
    </div>
<?php endif; ?>