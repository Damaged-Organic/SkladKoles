...
if( !$_AREA->{$C_AR::IS_AJAX_REQUEST} ) {
    ...
    $this->cancel_ajax_request();
} elseif( $_AREA->{$C_AR::AR_METHOD} !== "filter_data" ) {
    $this->cancel_ajax_request();
}
...
$items_filter = $this->load_inter("worker", "items_filter", $_AREA->{$C_E::_REQUEST}, $_AREA->{$C_AR::AR_METHOD}); //TODO: $_AREA->{$C_AR::AR_METHOD} - don't you need that!

if( !empty($_SESSION['filter_parameters']['filter_car_modification']) && !empty($items_filter['filter_modification']) ) {
