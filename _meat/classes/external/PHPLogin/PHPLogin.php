...
class PHPLogin
{
    ...
    public function __construct($db_handler, $users_table, $request_data)
    {
        $this->db_connection = $db_handler;
        ...
        if ( !empty($this->request_data['login_action']) && ($this->request_data['login_action'] === 'logout') ) {
            $this->doLogout();
            ...
            if (isset($this->request_data["user_edit_submit_name"])) {
                // function below uses use $_SESSION['user_id'] et $_SESSION['user_email']
                $this->editUserName($this->request_data['user_name']);
                // user try to change his email
            } elseif ( !empty($this->request_data['login_action']) && ($this->request_data['login_action'] === 'user_edit_submit_email') ) {
                // function below uses use $_SESSION['user_id'] et $_SESSION['user_email']
                if( isset($this->request_data['user_email']) ) $this->editUserEmail($this->request_data['user_email']);
                // user try to change his password
            } elseif ( !empty($this->request_data['login_action']) && ($this->request_data['login_action'] === 'user_edit_submit_password') ) {
                // function below uses $_SESSION['user_name'] and $_SESSION['user_id']
                if( isset($this->request_data['user_password_old']) &&
                    isset($this->request_data['user_password_new']) &&
                    isset($this->request_data['user_password_repeat']) ) {
                    $this->editUserPassword($this->request_data['user_password_old'], $this->request_data['user_password_new'], $this->request_data['user_password_repeat']);
                }
            }
        ...
        } elseif ( !empty($this->request_data['login_action']) && ($this->request_data['login_action'] === 'login') ) {
            if (!isset($this->request_data['user_rememberme'])) {
                $this->request_data['user_rememberme'] = null;
            }
            if (isset($this->request_data['user_email']) && isset($this->request_data['user_password']) ) {
                $this->loginWithPostData($this->request_data['user_email'], $this->request_data['user_password'], $this->request_data['user_rememberme']);
            }
        }

        // checking if user requested a password reset mail
        if ( isset($this->request_data['login_action']) && ($this->request_data['login_action'] === 'reset') && isset($this->request_data['user_email']) ) {
            $this->setPasswordResetDatabaseTokenAndSendMail($this->request_data['user_email']);
        } elseif (isset($this->request_data["user_name"]) && isset($this->request_data["verification_code"])) {
            $this->checkIfEmailVerificationCodeIsValid($this->request_data["user_name"], $this->request_data["verification_code"]);
        } elseif (isset($this->request_data["submit_new_password"])) {
            $this->editNewPassword($this->request_data['user_name'], $this->request_data['user_password_reset_hash'], $this->request_data['user_password_new'], $this->request_data['user_password_repeat']);
        }
    ...
    private function getUserData($user_name)
    {
        ...
        if ($this->db_connection) {
            ...
            $query_user = $this->db_connection->data_object->prepare("SELECT * FROM {$this->users_table} WHERE user_name = :user_name");
        ...
        if (isset($_COOKIE['rememberme'])) {
            ...
            if ($hash == hash('sha256', $user_id . ':' . $token . COOKIE_SECRET_KEY) && !empty($token)) {
                ...
                if ($this->db_connection) {
                    ...
                    $sth = $this->db_connection->data_object->prepare("SELECT user_id, user_name, user_email FROM {$this->users_table} WHERE user_id = :user_id
    ...
    private function loginWithPostData($user_email, $user_password, $user_rememberme)
    {
        ...
        // if POST data (from login form) contains non-empty user_email and non-empty user_password
            ...
            // if user has not typed a valid email address, stop
            if (!filter_var($user_email, FILTER_VALIDATE_EMAIL)) {
				$this->errors[] = MESSAGE_EMAIL_INVALID;
            } else if ($this->db_connection) {
				$user_email = filter_var($user_email, FILTER_SANITIZE_EMAIL);
                // database query, getting all the info of the selected user
				$query_user = $this->db_connection->data_object->prepare("SELECT * FROM {$this->users_table} WHERE user_email = :user_email");
				...
				// using PHP 5.5's password_verify() function to check if the provided passwords fits to the hash of that user's password
					...
					$sth = $this->db_connection->data_object->prepare("UPDATE {$this->users_table} "
				...
				// has the user activated their account with the verification email
					...
					if (defined('HASH_COST_FACTOR')) {
						...
						if (password_needs_rehash($result_row->user_password_hash, PASSWORD_DEFAULT, array('cost' => HASH_COST_FACTOR))) {
							...
							$query_update = $this->db_connection->data_object->prepare("UPDATE {$this->users_table} SET user_password_hash = :user_password_hash WHERE user_id = :user_id");
					...
					$this->messages[] = MESSAGE_LOGIN_SUCCESSFUL;
				}
			}
        }
    }
    ...
    private function newRememberMeCookie()
    {
        ...
        if ($this->db_connection) {
            ...
            $sth = $this->db_connection->data_object->prepare("UPDATE {$this->users_table} SET user_rememberme_token = :user_rememberme_token WHERE user_id = :user_id");
    ...
    private function deleteRememberMeCookie()
    {
        ...
        if ($this->db_connection) {
            ...
            $sth = $this->db_connection->data_object->prepare("UPDATE {$this->users_table} SET user_rememberme_token = NULL WHERE user_id = :user_id");
    ...
    public function doLogout()
    {
        ...
        header("Location: http://dem.skladkoles.dev");
        exit;
    }
    ...
    public function editUserName($user_name)
    {
        ...
        // TODO: maybe this pattern should also be implemented in Registration.php (or other way round)
            ...
            if (isset($result_row->user_id)) {
                ...
                $query_edit_user_name = $this->db_connection->data_object->prepare("UPDATE {$this->users_table} SET user_name = :user_name WHERE user_id = :user_id");
    ...
    public function editUserEmail($user_email)
    {
        ...
        // user mail cannot be empty and must be in email format
            ...
            if (isset($result_row->user_id)) {
                ...
                $query_edit_user_email = $this->db_connection->data_object->prepare("UPDATE {$this->users_table} SET user_email = :user_email WHERE user_id = :user_id");
    ...
    public function setPasswordResetDatabaseTokenAndSendMail($user_name)
    {
        ...
        if (empty($user_name)) {
            ...
            if (isset($result_row->user_id)) {
                ...
                // database query:
                $query_update = $this->db_connection->data_object->prepare("UPDATE {$this->users_table} SET user_password_reset_hash = :user_password_reset_hash,
    ...
    public function editNewPassword($user_name, $user_password_reset_hash, $user_password_new, $user_password_repeat)
    {
        ...
        // if database connection opened
            ...
            // write users new hash into database
            $query_update = $this->db_connection->data_object->prepare("UPDATE {$this->users_table} SET user_password_hash = :user_password_hash,
